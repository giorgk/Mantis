...
%====================================
disp("Data loaded. Waiting for command files")
...
while true
    ...
    try
        ...
        disp(strcat("Error encountered in processing. Resetting to retry. Error was of type:", ME.identifier));
        disp(strcat("Error message was: ", ME.message));
    end
end
...
function [MapID, RegIDs, LUinfo] = readInputfile(filename)
    ...
    try
        ...
        disp("Failed to work with input file");
        fclose(fid);
        rethrow(ME);  % make the error bubble up to the main loop - we just want to make sure to close the file handle so we can delete it later and it can be rewritten
    end
end
...
function writeoutfile(outfile, BTC)
    try
        ...
        disp("Problem writing output file - will try again");
        fclose(fid);
        rethrow(ME);  % make the error bubble up to the main loop - we just want to make sure to close the file handle so we can write it later
    end
end
